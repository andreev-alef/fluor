...
public class ReportSocGroup extends JasperReport {
...
//						+ "	--всего "
					...
					+ "		from examination  ex "
					+ "			join patient p on (p._ver_parent_id = ex.patient_id)and (p.soc_group_id = rsg.id) "
...
//						+ "	--Прочие "
					...
					+ "		where (ex.result_id > 3) and (p._ver_active) and (p.med_reg_id = pl.med_reg_id)and (p.med_city_id = pl.med_city_id) %s "
					+ "			and (ex.dat between ? and ?) "
					+ "		group by rsg.id) as proch "
					+ "from r_soc_group rsg "
					+ "	left join plan pl on (rsg.id = pl.soc_group_id) and (pl.med_reg_id = ?)and (pl.med_city_id = ?)and (pl.med_lpu_id  %s )"
//						+ "-- Осмотр физических лиц "
					+ "	left join ( "
					+ "			select p.soc_group_id, "
					...
					+ "					)) flren "
					+ "			from patient p "
					+ "			where (p._ver_active) and (soc_group_id is not null)and (p._ver_active) and (p.med_reg_id = ?)and (p.med_city_id = ?) %s "
					+ "			group by p.soc_group_id "
					+ "			order by p.soc_group_id "
					+ "	) ofl on (ofl.soc_group_id = rsg.id) "
//						+ "--Проведено исследований "
					...
					+ "			group by p.soc_group_id "
					+ "			order by p.soc_group_id "
					+ "		)prisl on (prisl.soc_group_id = rsg.id) "
					+ "order by rsg.id";
			String eq = "";
			String lpu ="";
			String gor = "";
