...
public class ReportDecrGroup extends JasperReport {
	...
	public void printReport(OutputStream output, ReportType type) throws JRException, IOException, SQLException, NamingException, ParseException {
		...
		try (Connection conn = getConnection()){
			String sql = "select rdg.id, rdg.name, cnt_beg_year, cnt_beg_year cnt_end_year, cnt_plan_pat, cnt_plan_exam, "
						...
						+ "	round(((ofl.flfl + ofl.flren)/cnt_beg_year *100),2) as oh_nas, "
//						+ "--Выявлено потологий "
//						+ "	--всего "
						+ "	(	select count (distinct patient_id) "
						+ "		from examination  ex "
						+ "			join patient p on (p._ver_parent_id = ex.patient_id)and (p.decr_group_id = rdg.id) "
						+ "			join r_exam_methods rem on (ex.method_id = rem.id) "
						+ "		where (ex.result_id > 1) and (p._ver_active) and (p.med_reg_id = pl.med_reg_id)and (p.med_city_id = pl.med_city_id) %s "
						+ "			and (ex.dat between ? and ?) "
						+ "		group by rdg.id) as vsego, "
//						+ "	-- Активный "
						+ "	(	select count (distinct patient_id) "
						+ "		from examination  ex "
						+ "			join patient p on (p._ver_parent_id = ex.patient_id)and (p.decr_group_id = rdg.id) "
...
//						+ "	--Прочие "
						...
						+ "		where (ex.result_id > 3)  and (p._ver_active) and (p.med_reg_id = pl.med_reg_id)and (p.med_city_id = pl.med_city_id) %s "
						+ "			and (ex.dat between ? and ?) "
						+ "		group by rdg.id) as proch "
						+ "from r_decr_group rdg "
						+ "	left join plan pl on (rdg.id = pl.decr_group_id) and (pl.med_reg_id = ?)and (pl.med_city_id = ?)and (pl.med_lpu_id  %s )"
//						+ "-- Осмотр физических лиц "
						+ "	left join ( "
						+ "			select p.decr_group_id, "
						...
						+ "					)) flren "
						+ "			from patient p "
						+ "			where (p._ver_active) and (decr_group_id is not null) and (p._ver_active) and (p.med_reg_id = ?)and (p.med_city_id = ?) %s "
						+ "			group by p.decr_group_id "
						+ "			order by p.decr_group_id "
						+ "	) ofl on (ofl.decr_group_id = rdg.id) "
//						+ "--Проведено исследований "
						...
						+ "			group by p.decr_group_id "
						+ "			order by p.decr_group_id "
						+ "		)prisl on (prisl.decr_group_id = rdg.id) "
						+ "order by rdg.id";
			String eq = "";
			String lpu ="";
			String gor = "";
